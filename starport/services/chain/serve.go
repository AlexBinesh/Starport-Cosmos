...
func homePage(w http.ResponseWriter, r *http.Request){
    ...
    //cmd := gexec.Command("starport", "relayer", "rly", "q", "txs" ,"althea-testnet1v4" , "message.action=transfer", "--offset", "1", "--limit", "1")
    log.Println("Executing this operation : ",operation)
    if (operation == "chains"){
	//EmpData := ""
	//var Chains []string
    	log.Println("Looking up  the CHAINS : ")
    	cmd := gexec.Command("starport", "relayer", "rly", "chains", "list" )
        out, err := cmd.Output(gexec.Dir("/tmp"))
        if err != nil {
                log.Fatal(err)
        }
	Chains := strings.Split(string(out), "\n")
	//fmt.Println("%s", "Chains")
	for i, rows := range Chains {
		//Line := strings.SplitAfter(rows, ":")
  	  ChainName := strings.Fields(rows)
	  //fmt.Println( i )
	  j := 0
	  for _, v := range ChainName {
    		if (j == 1){
	//	  fmt.Println(v)
		  if ( v != ""){
		  	Chains[i]=v
	...
	//fmt.Println("Range is ", range)
	RetVal, err := json.Marshal(Chains)
    	if err != nil {
    ...
    //cmd := gexec.Command("starport", "relayer", "rly", "q", "txs" ,network , "message.action=transfer", "--offset", "1", "--limit", limit)
    	log.Println("Looking up  the Balance for this netwrk: ",network)
    	cmd := gexec.Command("starport", "relayer", "rly", "q", "balance" ,network )
        out, err := cmd.Output(gexec.Dir("/tmp"))
        if err != nil {
                log.Fatal(err)
        }
    	...
    	log.Println("Looking up TXs for this netwrk: ",network)
...
func (c *Chain) start(ctx context.Context, config conf.Config) error {
	...
	fmt.Fprintf(c.stdLog(logStarport).out, "🌍 Running a Transaction Retrieval Server at  at PORT 10000 \n")
	handleRequests()
